...
export const startWorker = async (): Promise<void> => {
  ...
  worker = new Worker<ComplianceJobData>(
    ...
    async (job) => {
      ...
      try {
        // 1) Get Bedrock model for embeddings
        const bedrockModel = await getFirstAvailableBedrockModel();

        if (!bedrockModel) {
          throw BadRequest('This agent requires access to a Bedrock model for embeddings. Please configure a Bedrock AI provider first.');
        }
        ...
        const embeddingsAi = await ai.buildUserSource(bedrockModel.id);
