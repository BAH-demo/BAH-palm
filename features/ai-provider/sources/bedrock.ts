...
export class BedrockSource implements AiRepository {
  ...
  async createEmbeddings(input: string[], _config: AiSettings): Promise<AiResponse> {
    ...
    try {
      ...
      // but use Promise.all for parallel processing
      logger.debug(`Processing ${input.length} embeddings individually for model: ${modelId}`);

      const embeddingsResults = [];

      const embeddingPromises = input.map(async (text) => {
      ...
      embeddingsResults.push(...allResults);
