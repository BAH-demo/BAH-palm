...
describe('ChatAiProviderModelSelect', () => {
  ...
  it('shows correct tooltip text when model selection is disabled for existing chat', async () => {
    ...
    const selectElement = screen.getByTestId('model-select');
    const tooltipWrapper = selectElement.closest('div');
    
    await user.hover(tooltipWrapper!);

    await waitFor(() => {
      const tooltip = screen.getByText('Unable to change model selection once conversation has begun');
      expect(tooltip).toBeInTheDocument();
    });
  });

  it('shows correct tooltip text when chat model is unavailable', async () => {
    const user = userEvent.setup();
    ...
    await waitFor(() => {
      const tooltip = screen.getByText('The model associated with this chat has been deleted. Please start a new chat.');
      expect(tooltip).toBeInTheDocument();
    });
  });
});
