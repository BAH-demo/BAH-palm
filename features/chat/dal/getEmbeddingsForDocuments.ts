...
export default async function getEmbeddingsForDocuments({
  ...
  if (!documentIds.length) {
    logger.info('No document IDs provided, returning empty results');
    return [];
  }
  ...
  try {
    logger.info(`Querying embeddings for user ${userId} with provider ${documentLibraryProviderId}, threshold ${minThreshold}, document IDs: ${documentIds.join(', ')}`);
    ...
    logger.info(`Found ${rawResults.length} matching embeddings from specific documents`);
    ...
    logger.error('Error retrieving embeddings from specific documents', {
      userId,
      documentIds,
      minThreshold,
      matchCount,
