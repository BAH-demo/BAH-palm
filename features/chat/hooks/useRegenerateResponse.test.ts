...
describe('useRegenerateResponse', () => {
  ...
  beforeEach(() => {
    ...
    resetMessages();
    createMessage('Howdy!', MessageRole.User, stubChatId, 'test-user-message-id');
    createMessage('Hello', MessageRole.Assistant, stubChatId, stubMessageId);
  ...
  describe('Mutation Call', () => {
    it('calls the deleteMessage mutation with chat id and message id', async () => {
      const { result } = renderHook(() => useRegenerateResponse(stubMessageId));
      ...
      expect(deleteMessageMock).toHaveBeenCalledWith({
        chatId: stubChatId, messageId: stubMessageId,
      });
    });
    ...
    it('passes deepResearch flag when message is deep research', async () => {
      const deepResearchMessage = { ...messages[1], deepResearch: true };
      messages[1] = deepResearchMessage;
      getMessagesStub();
      ...
      expect(retryMessageMock).toHaveBeenCalledWith({
        chatId: stubChatId,
        customInstructions: expect.stringContaining(deepResearchMessage.content),
        knowledgeBaseIds: [],
        documentIds: [],
  ...
  describe('Error handling', () => {
    ...
    it('sets error if there is no chatId', () => {
      createChatProviderStub({ chatId: null, knowledgeBaseIds: [], documentIds: [], setRegeneratingResponse: setRegeneratingResponseMock });
      const { result } = renderHook(() => useRegenerateResponse(stubMessageId));
    ...
    it('sets error if message is not assistant message', () => {
      resetMessages();
      createMessage('Howdy!', MessageRole.System, stubChatId, 'test-system-message-id');
      createMessage('Hello', MessageRole.User, stubChatId, stubMessageId);
