...
import { AiProviderType, AiAgentType } from '@/features/shared/types';
...
export default function UserGroupAiAgentRow({
  ...
  const isCerta = aiAgent.type ===  AiAgentType.CERTA;
  ...
  const hasBedrockProvider = () => {
    ...
    return activeUserGroupProviders.some(
      provider => provider.typeId === AiProviderType.Bedrock
    );
  };
  ...
  const renderRequirementsContent = () => {
    ...
    const isRequirementMet = hasBedrockProvider();
    ...
    return (
      <Stack spacing='xs' w='100%'>
        <Group spacing='xs' noWrap>
          ...
          <Text size='sm'>AWS Bedrock Provider must be enabled for CERTA</Text>
        </Group>
      </Stack>
    );
  };
    ...
    <tr data-testid={`${aiAgent.id}-user-group-ai-agent-row`}>
      ...
      <td>
        <Group position='center'>
          <Switch
            aria-label={`Enable user group AI Agent ${aiAgent.label}`}
            checked={isEnabled}
            onChange={(event) =>
